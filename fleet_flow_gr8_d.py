# Eensure that CSAM facility deployment happens, by either a) increasing inter-m node transportation costs (m#_in -> m#_in line 114), or b) increasing demand (line 108), or c) reducing repair arc capacities on the traditional (L2) facilities (line 126), such that a deployment is forced to happen.
...
# Define sets
M = ['m1', 'm2', 'm3']
traditional_m_dict = {'k1': 'm1', 'k2': 'm2'}  # Type-specific traditional facilities
L = ['l1', 'l2']
K = ['k1', 'k2']
...
D = {(m, t, c): np.random.uniform(1, 10) * 3 for m in M for t in T for c in C}  # Tripled demand to force CSAM
print("\nDemands:")
for (m, t, c), d in D.items():
    print(f"D({m}, t={t}, {c}) = {d:.1f}")
# F = {m: np.random.uniform(100, 300) for m in M} # Original
F = {m: np.random.uniform(50, 150) for m in M} # Lower to encourage deployment
# C_in_in = np.random.uniform(2, 8)  # Travel cost
C_in_in = 25  # Increased to force local CSAM deployment
...
U_l2 = {k: 20 for k in K}  # Reduced to force CSAM for overflow
...
if model.status != 1:
    ...
    # Print dummy flows for unmet demand
