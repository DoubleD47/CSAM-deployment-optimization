...
Nodes: source, m_in, m_q_l{p}, m_r_l{p}, m_out_l{p}, sink, dummy (t=2), ss.
Decision Variables

$y_m \in {0,1}$ — deploy CSAM at $m$
$x_{ij,t,c} \geq 0$ — regular arc flows
$x_{ij,t,c,t'} \geq 0$ — queue carryover flows

Objective
$$\min \sum_{m \in M} F_m \, y_m + \sum c_{ij} \, x_{ij,\dots}$$
(Deployment + travel + queue entry + repair (l1/l2) + carryover + dummy penalty)
Main Constraints

Flow conservation at every node.
CSAM capacity: $\sum_{c:c[0]=l1} x_{(m_q_l1 \to m_r_l1),t,c} \leq U_{l1} \cdot y_m$
Traditional l2 capacity at fixed locations.
$\sum y_m \leq$ max facilities.

Full details in main.tex.
Benders Decomposition

Master: Binary $y$ + $\theta$ + optimality cuts.
Subproblem: Fixed-$y$ LP; duals from l1 capacity constraints feed cuts.
Manual PuLP loop for academic clarity.
...
│   └── Graphs_bar_charts_gr9_c_3_1.py
├── output/
├── viz_output/
├── main.tex
└── README.md
...
CSAM Deployed: m5, m8, m9
High unmet demand penalty (dummy flows) remains — good area for tuning.

See output/ and the generated charts in viz_output/.
Ongoing Work & Roadmap

 Stabilize Benders cuts / convergence
 Update visualization script for Benders output format
