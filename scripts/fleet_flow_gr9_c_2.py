...
# Demands (reduction to np.random.uniform(5, 15) to not overwhelm the system with unmet demand)
D = {}
for m in M:
    for t in T:
        for c in C:
...
model += (
    ...
    lpSum(C_dummy * x_regular[a] for a in regular_arcs if '_q_' in a[0] and 'dummy' in a[1]) +
...
dummy_cost = sum(value(C_dummy * x_regular[a]) for a in regular_arcs if '_q_' in a[0] and 'dummy' in a[1])
